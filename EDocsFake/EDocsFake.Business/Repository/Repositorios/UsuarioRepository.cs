...
using EDocsFake.Business.Repository.Interfaces;
using System.Collections.Generic;
using System.Linq;
namespace EDocsFake.Business.Repository.Repositorios
{
    public class UsuarioRepository : IUsuarioRepository
    {
        ...
        /// <returns></returns>
        public Entidades.Usuario AutenticarUsuario(string login, string senha)
        {
            using (var connection = new OracleConnection(_connectinStringOracle))
            {
                ...
                var usuario = connection.Query<Entidades.Usuario>(query, new { login = login, senha = senha }).FirstOrDefault();
                return usuario;
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<Entidades.Usuario> ListarTodosUsuarios()
        {
            using (var connection = new OracleConnection(_connectinStringOracle))
            {
                var query = "SELECT " +
                ...
                "FROM TB_USUARIO TU";
                var result = connection.Query<Entidades.Usuario>(query);
                return result;
            }
        }
